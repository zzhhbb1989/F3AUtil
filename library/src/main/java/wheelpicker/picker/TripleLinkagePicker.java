 ...
 */
public class TripleLinkagePicker extends LinkagePicker<Province, City, County> {
    private OnTriplePickListener onTriplePickListener;
    private OnDoublePickListener onDoublePickListener;
    ...
    public TripleLinkagePicker(Activity activity, List<Province> provinces) {
    ...
    public void setSelectedItem(String province, String city) {
        setSelectedItem(new Province(province), new City(city));
    }
    ...
    public County getSelectedCounty() {
        ...
        try {
            counties = getSelectedCity().getCounties();
            if (counties.size() == 0) {
    ...
    public void setOnTriplePickListener(OnTriplePickListener listener) {
        this.onTriplePickListener = listener;
    }
    ...
    /**
     * @deprecated use {@link #setOnTriplePickListener(OnTriplePickListener)} instead
     */
    @Deprecated
    @Override
    public final void setOnLinkageListener(OnLinkageListener onLinkageListener) {
        throw new UnsupportedOperationException("Please use setOnTriplePickListener instead.");
    }
    ...
    public void onSubmit() {
        ...
        if (!hideCounty) {
            ...
            if (onTriplePickListener != null) {
                onTriplePickListener.onPicked(province, city, county);
            }
        } else {
            if (onDoublePickListener != null) {
                onDoublePickListener.onPicked(province, city);
            }
        }
    }
    ...
    public interface OnDoublePickListener {
        void onPicked(Province province, City city);
    }
    
    /**
     * 三项选择回调
     */
    public interface OnTriplePickListener {
    ...
    private static class AddressProvider implements Provider<Province, City, County> {
        ...
        public List<County> linkageThirdData(int firstIndex, int secondIndex) {
            return thirdList.size()==0?null:thirdList.get(firstIndex).get(secondIndex);
        }
