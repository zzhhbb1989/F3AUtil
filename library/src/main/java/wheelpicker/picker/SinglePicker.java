...
import wheelpicker.entity.WheelItem;
...
public class SinglePicker<T> extends WheelPicker {
    ...
    private List<OnWheelListener<T>> onWheelListenerList = new ArrayList<>();
    private List<OnItemPickListener<T>> onItemPickListenerList = new ArrayList<>();
     ...
     * 设置滑动过程监听器
     */
    public void addOnWheelListener(OnWheelListener<T> onWheelListener) {
        onWheelListenerList.add(onWheelListener);
    }

    /**
     * 设置确认选择监听器
     */
    public void addOnItemPickListener(OnItemPickListener<T> listener) {
        onItemPickListenerList.add(listener);
    }
    ...
    protected View makeCenterView() {
        ...
        wheelView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
            ...
            public void onSelected(int index) {
                selectedItemIndex = index;
                for (OnWheelListener<T> listener : onWheelListenerList) {
                    listener.onWheeled(selectedItemIndex, items.get(index));
                }
            }
        });
    ...
    private String formatToString(Object item) {
     ...
     * @return
     */
    private String formatToString1(Object item) {
        WheelItem item1 = (WheelItem) item;
        return item1.getName();
    }

    @Override
    public void onSubmit() {
        for (OnItemPickListener<T> listener : onItemPickListenerList) {
            listener.onItemPicked(selectedItemIndex, getSelectedItem());
        }
    }
