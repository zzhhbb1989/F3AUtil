...
import wheelpicker.framework.util.DateUtils;
...
public class DateTimePicker extends WheelPicker {
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            LogUtils.verbose(this, "initList years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            LogUtils.verbose(this, "initList months before make view");
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            LogUtils.verbose(this, "initList days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
        ...
        if (timeMode != NONE && hours.size() == 0) {
            LogUtils.verbose(this, "initList hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            LogUtils.verbose(this, "initList minutes before make view");
    ...
    private int findItemIndex(ArrayList<String> items, int item) {
        ...
        if (index < 0) {
            throw new IllegalArgumentException("ItemData[" + item + "] out of range");
        }
        return index;
    }
